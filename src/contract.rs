...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::JoinGame {player_name, game_code } => try_join (deps, info, player_name, game_code),
        ExecuteMsg::SubmitChoice { game_code, choice} => try_submit_choice (deps, info,env, game_code, choice),
    }
}
...
pub fn query(
    ...
    match msg {
        QueryMsg::WhoWhon {game} => to_binary(&query_who_won(deps, env, game)?),
